...
const COINS = [
  ...
  { id: "usd-coin",     sym: "USDC", name: "USDC",      img: "https://assets.coingecko.com/coins/images/6319/small/usdc.png",      color: "#2775CA" },
...
function CoinDropdown({ selected, onSelect }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-3 w-full bg-gray-50 hover:bg-gray-100 transition-colors rounded-2xl px-4 py-3 border border-gray-200"
      >
        <img src={selected.img} alt={selected.sym} className="w-9 h-9 rounded-full flex-shrink-0" />
        <div className="text-left">
          <p className="text-gray-400 text-xs">Sell crypto</p>
          <p className="text-gray-900 text-sm font-bold">{selected.name}</p>
        </div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute left-0 right-0 top-full mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl z-50 overflow-hidden py-1"
          >
            {COINS.map((c) => (
              <button
                key={c.id}
              ...
              >
                <img src={c.img} alt={c.sym} className="w-7 h-7 rounded-full" />
                <div>
...
export default function SellCoin() {
  ...
  const fiatEquiv = price && numAmount ? (numAmount * price).toLocaleString("en-US", { maximumFractionDigits: 2 }) : "0";
        ...
        <div className="max-w-6xl mx-auto w-full px-6 sm:px-10 py-16 grid md:grid-cols-2 gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, ease: "easeOut" }}>
            ...
            <h1 className="text-4xl sm:text-5xl font-extrabold text-gray-900 leading-tight mb-5">
              Sell {coin.name} for Fiat.<br />Cash Out {coin.sym} in a<br />Few Clicks
            </h1>

            <p className="text-gray-500 text-base leading-relaxed mb-8 max-w-md">
              SaucamPro offers a simple, fast, and safe checkout to turn your {coin.sym} into cash. Easily sell {coin.name} for fiat into your bank account or card*. * Sell-to-Card available in over 80 countries. See full list below.
            </p>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <button onClick={() => navigate("/signup")} className="bg-gray-900 text-white font-semibold px-7 py-3.5 rounded-full hover:bg-gray-700 transition-colors text-sm">
                Get Started
              </button>
              <div className="flex items-center gap-3">
          ...
          <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, ease: "easeOut", delay: 0.1 }}>
            <div className="bg-white rounded-3xl border border-gray-100 shadow-xl p-6 sm:p-8 max-w-sm mx-auto">

              {/* Top controls */}
              <div className="flex items-center gap-2 mb-8">
                <CurrencyDropdown selected={currency} onSelect={setCurrency} />
                <span className="text-gray-500 text-sm font-medium ml-1">Sell crypto</span>
                <button className="ml-auto w-9 h-9 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition-colors text-gray-500">
              ...
              <div className="text-center mb-8">
                ...
                <div className="flex items-center justify-center gap-1.5 text-gray-400 text-sm">
                  ...
                  <span>{numAmount || "0"} {coin.sym}</span>
                </div>
              </div>
